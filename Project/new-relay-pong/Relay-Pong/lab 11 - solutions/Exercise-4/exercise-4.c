...
void * client_thread(void * arg){
    ...
    int c_sock_fd = (int) arg;
    char c;
    int n = 0;
    char message[100];
    
    while(read(c_sock_fd, &c, sizeof(c) == sizeof(c))){
        //printf("new_char\n");
        sprintf(message,"%d :%c", n, c);
        safe_mvwprintw(small_win, 2,1,message); 
        sleep(1);
        n++;
    }
...
void * server_thread(void *arg){
    ...
    while(1){
        ...
        pthread_create(&client_t_id, NULL, client_thread, (void *)client_fd);
...
{	
    ...
    pthread_create(&server_t_id, NULL, server_thread, NULL);
    int key;
    int n =0 ; 
    char message[100];
    do
